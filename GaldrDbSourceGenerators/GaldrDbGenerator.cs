...
namespace GaldrDbSourceGenerators
{
    ...
    public class GaldrDbGenerator : IIncrementalGenerator
    {
        ...
        private static string GenerateRegistry(List<ClassInfo> classes, Dictionary<string, string> resolvedNames)
        {
            ...
            sb.AppendLine("    /// </summary>");
            sb.AppendLine("    public static class GaldrTypeRegistry");
            sb.AppendLine("    {");

            // Static dictionary
            sb.AppendLine("        private static readonly Dictionary<Type, IGaldrTypeInfo> _registry = new Dictionary<Type, IGaldrTypeInfo>");
            sb.AppendLine("        {");

            foreach (ClassInfo classInfo in classes)
            {
                string resolvedName = resolvedNames[classInfo.FullyQualifiedName];
                sb.AppendLine($"            {{ typeof({classInfo.FullyQualifiedName}), {resolvedName}Meta.TypeInfo }},");
            }
            ...
            // Get<T> method
            sb.AppendLine("        /// <summary>");
            sb.AppendLine("        /// Gets type info for the specified type.");
            sb.AppendLine("        /// </summary>");
            sb.AppendLine("        public static GaldrTypeInfo<T> Get<T>()");
            sb.AppendLine("        {");
            sb.AppendLine("            return (GaldrTypeInfo<T>)_registry[typeof(T)];");
            sb.AppendLine("        }");
            sb.AppendLine();

            // Get(Type) method
            sb.AppendLine("        /// <summary>");
            sb.AppendLine("        /// Gets type info for the specified type (non-generic).");
            sb.AppendLine("        /// </summary>");
            sb.AppendLine("        public static IGaldrTypeInfo Get(Type type)");
            sb.AppendLine("        {");
            sb.AppendLine("            return _registry[type];");
            sb.AppendLine("        }");
            sb.AppendLine();

            // TryGet<T> method
            sb.AppendLine("        /// <summary>");
            sb.AppendLine("        /// Tries to get type info for the specified type.");
            sb.AppendLine("        /// </summary>");
            sb.AppendLine("        public static bool TryGet<T>(out GaldrTypeInfo<T> info)");
            sb.AppendLine("        {");
            sb.AppendLine("            if (_registry.TryGetValue(typeof(T), out IGaldrTypeInfo typeInfo))");
            sb.AppendLine("            {");
            sb.AppendLine("                info = (GaldrTypeInfo<T>)typeInfo;");
            ...
            // GetAll method
            sb.AppendLine("        /// <summary>");
            sb.AppendLine("        /// Returns all registered type infos.");
            sb.AppendLine("        /// </summary>");
            sb.AppendLine("        public static IEnumerable<IGaldrTypeInfo> GetAll()");
            sb.AppendLine("        {");
            sb.AppendLine("            return _registry.Values;");
            sb.AppendLine("        }");
