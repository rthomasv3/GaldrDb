#!/bin/bash
# Build script for creating GaldrDb Browser AppImage with manual dependency bundling
...
echo "=== Building GaldrDb Browser AppImage ==="
...
dotnet publish "$BROWSER_DIR/GaldrDbBrowser.csproj" \
    -c Release \
    -r linux-x64 \
    --self-contained true \
...
cp "$PUBLISH_DIR/GaldrDbBrowser" "$APPDIR/usr/bin/"
...
#   /usr/libexec/webkit2gtk-4.1 → /tmp/galdrdb-webkit/libexec
#   /usr/lib64/webkit2gtk-4.1/injected-bundle/ → /tmp/galdrdb-webkit/lib64/injected-bundle/
python3 "$SCRIPTS_DIR/patch-webkit.py" "$APPDIR"

# Verify patches worked
echo "Verifying patches:"
strings "$APPDIR/usr/lib/libwebkit2gtk-4.1.so.0" | grep -E "(galdrdb-webkit|/tmp)" | head -5 || echo "⚠ Warning: Could not verify patches"
...
# - Creates symlinks in /tmp/galdrdb-webkit/ for WebKit helper processes
# - Executes the application
"$SCRIPTS_DIR/create-apprun.sh" "$APPDIR"

# Copy desktop file and icons
cp "$CONFIG_DIR/io.github.rthomasv3.GaldrDbBrowser.desktop" "$APPDIR/galdrdb-webkit.desktop"
# Note: You need to provide an icon file at assets/icons/io.github.rthomasv3.GaldrDbBrowser.256.png
if [ -f "$ASSETS_DIR/icons/io.github.rthomasv3.GaldrDbBrowser.256.png" ]; then
    cp "$ASSETS_DIR/icons/io.github.rthomasv3.GaldrDbBrowser.256.png" "$APPDIR/.DirIcon"
    cp "$ASSETS_DIR/icons/io.github.rthomasv3.GaldrDbBrowser.256.png" "$APPDIR/io.github.rthomasv3.GaldrDbBrowser.png"
else
    echo "⚠ Warning: Icon file not found at $ASSETS_DIR/icons/io.github.rthomasv3.GaldrDbBrowser.256.png"
    # Create a placeholder icon
    touch "$APPDIR/.DirIcon"
    touch "$APPDIR/io.github.rthomasv3.GaldrDbBrowser.png"
fi
...
rm -f "$OUTPUT_DIR/GaldrDbBrowser-x86_64.AppImage"
...
ARCH=x86_64 APPIMAGE_EXTRACT_AND_RUN=1 "$TOOLS_DIR/appimagetool-x86_64.AppImage" \
    --no-appstream \
    "$APPDIR" \
    "$OUTPUT_DIR/GaldrDbBrowser-x86_64.AppImage"
...
# ============================================================================
echo -e "\n=== Build Complete ==="
ls -lh "$OUTPUT_DIR/GaldrDbBrowser-x86_64.AppImage"
echo -e "\nAppImage location: $OUTPUT_DIR/GaldrDbBrowser-x86_64.AppImage"
echo -e "Size: $(du -h $OUTPUT_DIR/GaldrDbBrowser-x86_64.AppImage | cut -f1)"
echo -e "\nHow it works:"
echo "  1. Manual ldd-based bundling with excludelist for system libraries"
echo "  2. WebKit library is binary-patched to look for helpers in /tmp/galdrdb-webkit/"
...
echo -e "\nTest with: $OUTPUT_DIR/GaldrDbBrowser-x86_64.AppImage"
